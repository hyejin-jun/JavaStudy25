	...
	public static void main(String[] args) {
		...
		while (run) { // 메뉴 while
			...
			case 'A':
				...
				while (Loginrun) {
					System.out.println(" ====== 1을 누르시면 회원가입 메뉴로 이동합니다 !! ♡ ======");
					System.out.println(" ====== 2를 누르시면 로그인 메뉴로 이동합니다 !! ♡ ======");
					...
					case 1:
						...
						System.out.print("회원가입하실 회원 수를 입력해 주세요 >>> ");

						int jj = inputInt.nextInt();
						String[] joinId = new String[jj];
						String[] joinPw = new String[jj];
						String[] joinName = new String[jj];

						joinHw(joinId, joinPw, joinName); {
			...
			case 'b':
				...
				while (artRun) { // 아티스트 while
					...
					case 1:
						...
						System.out.print("입력하실 아티스트 수를 입력해 주세요  >> ");
						int dd = inputInt.nextInt();
						String[] artName = new String[dd];
						String[] artSs = new String[dd];
						String[] artPr = new String[dd];

						System.out.println("아티스트 등록 메뉴로 이동하였습니다 !! ♡ ");

						artJoin(artName, artSs, artPr); {
...
private static void joinHw(String[] joinId, String[] joinPw, String[] joinName) {	// 회원가입 창 메서드 
	...
	for (int i = 0 ; i < joinId.length ; i++) {
		...
		joinId [i] = inputStr.next( ) ;
		System.out.print("회원가입 비밀번호를 입력하세요 !! ♡ >> ");
		joinPw [i] = inputStr.next( ) ;
		System.out.print("회원가입할 회원님의 성함을 입력하세요 !! ♡ >> ");
		joinName [i] = inputStr.next( ) ;
		
	}  // 회원가입 폴 종료
		 
	}  // 회원가입 메서드 종료

	private static void artJoin(String[] artName, String[] artSs, String[] artPr) {
		// TODO Auto-generated method stub
		Scanner inputStr = new Scanner(System.in);

		for (int i = 0; i < artName.length; i++) {

			System.out.print("등록할 아티스트의 성함을 입력하세요 !! ♡ >> ");
			artName[i] = inputStr.next();
			System.out.print("등록할 아티스트의 성별을 입력하세요 !! ♡ >> ");
			artSs[i] = inputStr.next();
			System.out.print("등록할 아티스트의 수상 경력을 입력하세요 !! ♡ >> ");
			artPr[i] = inputStr.next();
		}
	}
