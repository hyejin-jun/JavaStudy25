...
public class ArrayV2Exam {
	...
	public static void main(String[] args) {
		...
		int select = 0;
		System.out.print("초기 작업: 학생 수를 입력하세요 >>  ");

		int count = mainSC.nextInt();

		String[] name = new String[count]; // 학생 수로 학생 테이블 작성

		byte[] engScores = new byte[count]; // 영어 성적 테이블
		byte[] korScores = new byte[count]; // 국어 성적 테이블
		int[] totalScores = new int[count]; // 영어 성적 테이블
		double[] aveScores = new double[count]; // 영어 성적 테이블

		boolean run = true;
		while (run) {
			System.out.println("===== 엠비씨 교육센터 성적 관리 프로그램 ===== ");
			System.out.println("1 학생 관리 / 2 성적 관리 / 3 통계 / 4 종료");
			System.out.print(">>>>>>>>>   ");
			select = mainSC.nextInt(); // 1부터 4까지 정수 입력 후에 분기
			...
			case 4 -> {
				System.out.println("성적 처리 고생하셧습니다");
				run = false;
			} // 케이스 4 종료
			} // 주메뉴 스위치 종료
		} // 와일문 종료
...
// default :
	...
	private static void totalScores(int[] totalScores, byte[] engScores, byte[] korScores, String[] name) { // 총합 스코어
		...
		while (run) {
			...
			System.out.println("1 전체 성적 관리 / 2 개인 성적 검색 / 3 메인 메뉴");
			System.out.print(" >>>>>>>>> ");
			...
			case 3:
				...
				break;
			default:
				System.out.println("번호를 잘못 입력하엿습니다");
	...
	private static void totalScores2(int[] totalScores, byte[] engScores, byte[] korScores, String[] name) {
		...
		for (int i = 0; i < totalScores.length; i++) {
			...
			System.out.println(name[i] + "학생의 총점:  " + totalScores[i] + "/ 평균: " + totalScores[i / 2]);
		} // 합계 및 평균
	...
	private static void searchScores2(int[] totalScores, byte[] engScores, byte[] korScores, String[] name) {
		...
		System.out.print("검색하실 학생의 번호를 입력해 주세요 >>>  ");
		...
		System.out.println("총합 점수:  " + totalScores[i - 1] + "점");
		System.out.println("평균 점수:  " + totalScores[i - 1 / 2] + "점");
		...
		System.out.println("클래스에서의 순위는:" + rank + "  위입니다");
