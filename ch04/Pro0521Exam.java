...
public class Pro0521Exam {
	...
	public static void main(String[] args) { // 메인 메서드
		...
		String [] ids = null;		// 회원가입 아이디
		String [] pws = null; 	 // 회원가입 패스워드
		String [] names = null;	// 회원가입 이름
		Member [] gaedes = null ;  	// 회원가입 등급
		...
		while (run) { // pp 와일 시작
			...
			case 'a':
				...
				while (jprun) { // 관리자 메뉴 와일 시작
					...
					case 1:
						...
						session = loginOK(adminId, adminPw, session);

						if (session == true) {
							System.out.println(adminId + " 님 환영합니다 !! ♡ ");
						} else {
			...
			case 'b':
				...
				while (pmrun) {// 와일 시작
					...
					System.out.println("0. 회원가입 메뉴로 이동하겠습니다 !! ♡");
					...
					System.out.println("5. 금액 충전 메뉴로 이동합니다 !! ♡") ;
					...
					case 0:
						...
						System.out.print("가입할 인원 수를 입력해 주세요 >>> ");
						int jj = inputInt.nextInt();
						
						String[] ids1 = new String[jj];
						String[] pws1 = new String[jj];
						String[] names1 = new String[jj];
						Member[] grades = new Member[jj];

						joinHw(ids1, pws1, names1, grades); {
					...
					case 3:
						System.out.print("랜덤이라는 글자를 입력하시면 랜덤 박스가 지정이 됩니다! >> ");
						String ovv = inputStr.next(); // 글자 적을 수 있는 칸을 생성
						
						rondum (totalnn, inputStr, ovv);
						break;
					...
					case 5:
						System.out.println("금액 충전 메뉴로 이동하였습니다 !! ♡ ");	
						int[] scores = {1000, 5000, 10000, 50000} ;
						point(scores) ;
	...
	} private static void point(int[] scores) {  // 포인트 충전 메서드 생성
		...
		System.out.println("1. 1,000 / 2. 5,000 / 3. 10,000 / 4. 50,000 ");
	...
	private static void rondum(String totalnn, Scanner inputString1, String ovv) {  // 랜덤 박스 메서드 생성
		
		Scanner inputStr = new Scanner (System.in) ;
		
		String totalnn1 = "랜덤"; // 랜덤 글자를 입력하면 랜덤 박스가 나오게 인식
	...
	private static void joinHw(String[] ids1, String[] pws1, String[] names1, Member[] grades) {  // 회원가입 메서드 생성
		...
		for (int i = 0 ; i < ids1.length ; i++) {
			...
			ids1 [i] = inputStr.next( ) ;
			System.out.print("회원가입 비밀번호를 입력하세요 !! ♡ >> ");
			pws1 [i] = inputStr.next( ) ;
			System.out.print("회원가입할 회원님의 성함을 입력하세요 !! ♡ >> ");
			names1 [i] = inputStr.next( ) ;
			System.out.print("회원가입할 회원님의 등급을 입력하세요 !! ♡ >> ");
			System.out.print("1. 	GUEST / 2. MEMBER / 3. VIP / 4. VVIP / 5. VVVIP") ;
			int grades11 = inputInt.nextInt() ;
			
			switch (grades11) {
			case 1:
			grades[i] = Member.GUEST ;
			System.out.println("고객님의 등급은 게스트입니다" );
			break ;
			
			case 2:
			grades[i] = Member.MEMBER ;
			System.out.println("고객님의 등급은 게스트입니다" );
			break ;
			...
			case 4:
			grades[i] = Member.VVIP ;
			System.out.println("고객님의 등급은 게스트입니다" );
			break ;
			
			case 5:
			grades[i] = Member.VVVIP ;
			System.out.println("고객님의 등급은 게스트입니다" );
			break ;
	...
	static boolean loginOK(String adminId, String adminPw, boolean session) { // 관리자 로그인 메서드
		...
		if (loginid.equals(adminId) & loginpw.equals(adminPw)) {
